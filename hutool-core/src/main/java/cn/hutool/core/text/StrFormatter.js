 ...
 * @author Looly
 */
export default class StrFormatter {
	 ...
	 * @return 结果
	 */
	static /*String*/ format(/*String*/ strPattern,... argArray) {
		return this.formatWith(strPattern, '{}', argArray);
	}
	 ...
	 * @since 5.7.14
	 */
	 static /*String*/ formatWith(strPattern, placeHolder, ... argArray) {
		if (strPattern == null || placeHolder == null || argArray == null || argArray.length == 0) {
			return strPattern
		}
	 ...
	 */
	 static /*String*/ format(template, map, ignoreNull) {
		...
		let template2 = template.toString();
		for (let key in map) {
			let value = map[key]
			if (null == value && ignoreNull) {
				continue;
			}
			template2 = template2.replace( "{" + key + "}", value);
		}
		return template2;
	}
}
